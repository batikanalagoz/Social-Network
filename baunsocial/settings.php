...
<div class="settings-wrap">
    <div class="user-setting-wrap justify" style="border-top-left-radius:5px; border-top-right-radius:5px; background-color: gainsboro;">
        <h3 class="align-middle" style="color: #3f51b5">Ayarlar</h3>
    </div>
    <div class="user-name-wrap justify" data-userid="<?php echo $userid; ?>">
        <div class="sett-head">Kullanıcı adını değiştir</div><span class="set-changed-name"><?php echo ''.$userData->first_name.' '.$userData->last_name.''; ?></span>
    </div>
    <div class="user-link-wrap justify" data-userid="<?php echo $userid; ?>">
        <div class="sett-head">Kullanıcı Bağlantısını Değiştir</div><span class="set-changed-userLink"><?php echo $userData->userLink; ?></span>
    </div>
    <div class="mobile-number-wrap justify" data-userid="<?php echo $userid; ?>">
       <?php if(!empty($userData->mobile)){ ?>
        <div class="sett-head">Cep Numarasını Değiştir</div><span class="set-changed-mobile"><?php echo $userData->mobile; ?></span>
        <?php }else{?>
        <div class="sett-head">Cep Numarasını Değiştir</div><span class="set-changed-mobile">Hiçbir cep telefonu numarası bulunamadı. Bir tane ekleyin.</span>
          <?php } ?>
    </div>
      <div class="email-id-wrap justify" data-userid="<?php echo $userid; ?>">
       <?php if(!empty($userData->email)){ ?>
        <div class="sett-head">E-postanızı Değiştirin</div><span class="set-changed-email"><?php echo $userData->email; ?></span>
        <?php }else{?>
        <div class="sett-head">E-postanızı Değiştirin</div><span class="set-changed-email">Hiçbir e-posta bulunamadı. Bir tane ekleyin.</span>
          <?php } ?>
    </div>
    <div class="user-password-wrap justify" data-userid="<?php echo $userid; ?>">
        <div class="sett-head">Şifre değiştir</div><span class="set-changed-password">*******</span>
    </div>
...
$(function(){
    $(document).on('click','.user-name-wrap', function(){
        ...
        $('.top-box-show2').html('<div class="change-input-wrap"><div class="change-input"><h3>Adınızı değiştirin</h3><label for="first-name-change">Isim: </label><input type="text" name="" id="first-name-change" value="'+firstName+'" class="input-style"><label for="last-name-change"> Soyad: </label><input type="text" name="" id="last-name-change" value="'+lastName+'" class="input-style"><br><br><input type="submit" value="Submit" id="name-change-submit" data-userid="'+userid+'" class="input-style"></div></div>');
    ...
    $(document).on('click', '#name-change-submit', function(){
        ...
        if(firstName == '' || lastName == ''){
            ...
            $.post('http://localhost/baunsocial/core/ajax/settings.php', {
                changeName: userid,
                firstName : firstName,
                lastName : lastName
            }, function(data){
        ...
        if(userLink == ''){
            ...
            $.post('http://localhost/baunsocial/core/ajax/settings.php', {
                ...
                userid: userid
                    ...
                    if(data.trim() == '<h3 style="color:#4caf50;">Kullanıcı Bağlantısı başarıyla değiştirildi.</h3>'){
 ...
 $(document).on('click','.mobile-number-wrap', function(){
        ...
        if(mobileChange == ''){
            ...
            $.post('http://localhost/baunsocial/core/ajax/settings.php', {
                ...
                userid: userid
                    ...
                    if(data.trim() == '<h3 style="color:#4caf50;">Cep telefonu numarası başarıyla değiştirildi.</h3>'){
    ...
    $(document).on('click','.user-password-wrap', function(){
        var userid = $(this).data('userid');
        $('.top-box-show2').html('<div class="change-input-wrap"><div class="change-input"><h3>Şifre değiştir</h3><label for="old-password">Eski Şifre: </label><input type="password" name="" id="old-password" value="" class="input-style"><label for="new-password">Yeni Şifre: </label><input type="password" name="" id="new-password" value="" class="input-style"><br><br><input type="submit" value="Submit" id="password-submit" data-userid="'+userid+'" class="input-style"></div></div>');
